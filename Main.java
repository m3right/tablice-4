...
public class Main {
    ...
    public static void babelekUlepszony(int[] tablica) {
        int tmp;
        int i;
        boolean tablicaPosortowana = true;

        while (tablicaPosortowana) {
            tablicaPosortowana = false;
            for (i = 1; i < tablica.length; i++) {
                if (tablica[i - 1] > tablica[i]) {
                    tmp = tablica[i - 1];
                    tablica[i - 1] = tablica[i];
                    tablica[i] = tmp;
                    tablicaPosortowana = true;
                }
            }
        }
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println("Babelek ulepszony: ");
            babelekUlepszony(t);
            wypisz_tablice(t, r);
          
            Arrays.sort(arraySort);
            System.out.println("Uzycie arrays.sort:" + Arrays.toString(arraySort));
